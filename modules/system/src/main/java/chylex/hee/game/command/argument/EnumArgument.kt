...
class EnumArgument<T : Enum<T>>(private val enumClass: Class<T>) : ArgumentType<T> {
	...
	object Serializer : IArgumentSerializer<EnumArgument<*>> {
		override fun write(argument: EnumArgument<*>, buffer: PacketBuffer) {
			buffer.writeString(argument.enumClass.name, 256)
		}
		...
		override fun read(buffer: PacketBuffer): EnumArgument<*> {
			return EnumArgumentConstructor.createEnumArgument(Class.forName(buffer.readString(256)))
		}
	}
