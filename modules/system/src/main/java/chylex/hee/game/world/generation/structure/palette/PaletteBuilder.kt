package chylex.hee.game.world.generation.structure.palette
	...
	fun add(key: String, baseState: BlockState, propertyMappings: List<Pair<Property<*>, Map<String, *>>>) {
		fun process(index: Int, currentKey: String, currentState: BlockState) {
			...
			for ((name, value) in mapping) {
				process(index + 1, updateKey(currentKey, name), BlockStateGenerics.withProperty(currentState, property, value as Comparable<*>))
			}
		}
