...
import chylex.hee.game.entity.damage.DamageProperties.Reader
import chylex.hee.game.entity.damage.IDamageDealer.Companion.CANCEL_DAMAGE
	...
	companion object {
		...
		val PEACEFUL_EXCLUSION = object : IDamageProcessor {
			override fun modifyDamage(amount: Float, target: Entity, properties: DamageProperties.Reader): Float {
				return if (target.world.difficulty != PEACEFUL || target !is PlayerEntity)
					amount
				else
					CANCEL_DAMAGE
			}
		...
		val DIFFICULTY_SCALING = object : IDamageProcessor {
			override fun modifyDamage(amount: Float, target: Entity, properties: DamageProperties.Reader): Float {
				return if (target !is PlayerEntity) {
					amount
				}
		...
		val POTION_PROTECTION = object : IDamageProcessor {
			...
			override fun modifyDamage(amount: Float, target: Entity, properties: DamageProperties.Reader): Float {
				return if (target !is LivingEntity)
					...
					amount * (1F - (0.15F * (target.getActivePotionEffect(Effects.RESISTANCE)?.amplifier?.plus(1)?.coerceAtMost(5) ?: 0)))
			}
		}
		
		@JvmStatic
		fun STATUS(effect: (LivingEntity) -> EffectInstance?) = object : IDamageProcessor {
