...
import chylex.hee.game.territory.TerritoryType.Companion.CHUNK_MARGIN
import chylex.hee.game.territory.TerritoryType.Companion.CHUNK_X_OFFSET
import chylex.hee.game.territory.TerritoryType.THE_HUB
...
data class TerritoryInstance(val territory: TerritoryType, val index: Int) {
	...
	fun getSpawnPoint(player: PlayerEntity): BlockPos {
		return storage?.loadSpawnForPlayer(player) ?: fallbackSpawnPoint
	}
	
	fun updateSpawnPoint(player: PlayerEntity, pos: BlockPos) {
	...
	private fun determineOwningPlayer(entity: Entity) = when (entity) {
		...
		is ItemEntity     -> entity.throwerId?.let(Environment.getServer().playerList::getPlayerByUUID)
		is TameableEntity -> entity.owner as? PlayerEntity
		else              -> null
	}
}
