...
class TerritoryEntry(private val owner: TerritoryGlobalStorage, private val instance: TerritoryInstance, val type: TokenType) : INBTSerializable<TagCompound> {
	...
	fun loadSpawn(): BlockPos {
		if (spawnPoint == null) {
			val info = (TerritoryInstance.endWorld.chunkProvider.chunkGenerator as EndChunkGenerator).getGenerationInfo(instance)
	...
	fun loadSpawnForPlayer(player: PlayerEntity): BlockPos {
		return lastPortals[player.uniqueID] ?: loadSpawn()
	}
	
	fun updateSpawnForPlayer(player: PlayerEntity, pos: BlockPos) {
		if (pos == spawnPoint) {
