...
class ChunkGeneratorEndCustom(private val biomeRegistry: Registry<Biome>, private val seed: Long) : ChunkGenerator(SingleBiomeProvider(biomeRegistry.getOrThrow(Biomes.THE_END)), DimensionStructuresSettings(false)) {
	companion object {
		private val CODEC: Codec<ChunkGeneratorEndCustom> = RecordCodecBuilder.create {
			val biomeRegistry = RegistryLookupCodec.getLookUpCodec(Registry.BIOME_KEY).forGetter(ChunkGeneratorEndCustom::biomeRegistry)
			val seed = Codec.LONG.fieldOf("seed").forGetter(ChunkGeneratorEndCustom::seed)
			it.group(biomeRegistry, seed).apply(it, ::ChunkGeneratorEndCustom)
		}
	...
	override fun func_230349_a_(seed: Long): ChunkGenerator {
		return ChunkGeneratorEndCustom(biomeRegistry, seed)
	}
