...
object RepairHandler{
	...
	fun onAnvilUpdate(e: AnvilUpdateEvent){
		...
		if (item is ICustomRepairBehavior && item.getIsRepairable(target, ingredient)){
			...
			if (instance.repaired.isEmpty || instance.experienceCost > MAX_EXPERIENCE_COST){ // TODO check if player is in creative for xp limit, but I can't even do that with the event... replace all this shit with ASM
				e.output = ItemStack.EMPTY
				e.cost = 0
			}
			else{
				e.output = instance.repaired.also { it.repairCost = instance.repairCost } // TODO cannot repair & change name at the same time, but CBA
